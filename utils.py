...
class LoadImages:  # for inference
    def __init__(self, path, one_file, img_size=640, stride=32):
    ...
    def __next__(self):
        ...
        img0 = cv2.resize(img0, (1280,704), interpolation=cv2.INTER_LINEAR)
...
def save_masks(da_seg_mask: np.ndarray, ll_seg_mask: np.ndarray, save_path):
    print(da_seg_mask.shape, ll_seg_mask.shape)
    da_seg_mask[da_seg_mask == 1] = 255
    ll_seg_mask[ll_seg_mask == 1] = 255
    
    cv2.imwrite(str(save_path) + '/da_seg_mask.jpg', da_seg_mask)
    cv2.imwrite(str(save_path) + '/ll_seg_mask.jpg', ll_seg_mask)
